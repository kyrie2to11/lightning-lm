...
class ImuProcess {
   public:
    ...
    void SetInitialWorldImuRotation(const Mat3d &R_world_imu) {
        use_initial_world_imu_rotation_ = true;
        initial_R_world_imu_ = R_world_imu;
    }
   ...
   private:
    ...
    bool use_initial_world_imu_rotation_ = false;
    Mat3d initial_R_world_imu_ = Mat3d::Identity();
    IMUFilter filter_;
};
...
inline void ImuProcess::UndistortPcl(const MeasureGroup &meas, ESKF &kf_state, CloudPtr &pcl_out,
    ...
    if (!deskew_points) {
        // Polka path: deskew skipped (pre-deskewed cloud), transform points
        // from LiDAR/base_footprint frame to IMU body frame so that
        // PointBodyToWorld/ObsModel don't need extrinsic offset.
        pcl_out = meas.scan_;
        for (auto& pt : pcl_out->points) {
            Vec3d p(pt.x, pt.y, pt.z);
            Vec3d p_imu = R_lidar_imu_ * p + t_lidar_mu_;
            pt.x = p_imu(0);
            pt.y = p_imu(1);
            pt.z = p_imu(2);
        }
