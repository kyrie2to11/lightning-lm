...
class PangolinWindowImpl {
   ...
   public:
    ...
    std::atomic<bool> optimized_pgo_need_update_{false};  // PGO 优化帧
    ...
    SE3 optimized_pgo_pose_;                                  // 低频 PGO 优化 pose
    SE3 predicted_pose_;
   ...
   private:
    ...
    /// cloud rendering
    ui::UiCar backend_car_{Vec3f(0.2, 0.2, 0.8), 2.0f, 3.0f, /*dashed=*/true};   // 后端：虚线
    ui::UiCar frontend_car_{Vec3f(0.2, 0.2, 0.8), 2.0f, 3.0f};  // 前端：实线
    ...
    std::shared_ptr<ui::UiTrajectory> traj_optimized_pgo_ = nullptr; // PGO 优化帧轨迹
